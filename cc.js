...
function turn1(){//hit1 enable, hit2 disable
...
function turn2(){//hit1disable and hit2 enable
...
{    
		...
		{
								 var a=document.getElementById("randm1").innerHTML = (Math.floor(Math.random() * 100) + 1);
...
function random2()
{
		...
		{
		var b= document.getElementById("randm2").innerHTML = (Math.floor(Math.random() * 100) + 1); 
...
alert(' Team A ! ur trun gets over');
}
...
function blastturn(){  
if(chanceleft2.value=="0"&&chanceleft1.value=="1"){//team a last turn 
} 
else if(chanceleft2.value=="0"&&chanceleft1.value>"0"){
alert(' Team B ! ur turn gets over');
}
